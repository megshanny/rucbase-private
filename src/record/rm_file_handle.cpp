...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    context->lock_mgr_->lock_shared_on_record(context->txn_, rid, fd_); //读取记录时加读锁
    RmPageHandle page_handle = fetch_page_handle(rid.page_no); //获取指定记录所在的page handle
    char* record_data = page_handle.get_slot(rid.slot_no); //定位记录在页面中的具体位置
    auto record = std::make_unique<RmRecord>(file_hdr_.record_size, record_data); //初始化一个指向RmRecord的指针
    return record;
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    // 查找页面中的第一个空闲槽位
    int slot_no = Bitmap::first_bit(0, page_handle.bitmap, file_hdr_.bitmap_size);

    // 创建记录号（页面号 + 槽位号）
    Rid new_record_id = {page_handle.page->get_page_id().page_no, slot_no};

    // 若当前有事务，则为该记录加写锁
    if (context->txn_ && !context->lock_mgr_->lock_exclusive_on_record(context->txn_, new_record_id, fd_)) {
        return Rid{-1, -1}; // 锁失败，返回无效记录号
    }
    ...
    // 若页面已满，则更新文件头的 first_free_page_no
    if (page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
        if (page_handle.page_hdr->next_free_page_no == -1) {
            file_hdr_.first_free_page_no = -1; // 没有空闲页面
        } else {
            file_hdr_.first_free_page_no = page_handle.page_hdr->next_free_page_no; // 指向下一个空闲页面
        }
    }

    return new_record_id; // 返回新插入记录的位置
}
